...
int main(int argc, char** argv) {
    ...
    for (int test_case = 0; test_case < test_count; ++test_case) {
        long long n = n_list[test_case],k=rnd.wnext(1LL,1000000000LL, value_bias),y=rnd.wnext(0LL,1000000000LL, value_bias);
        vector<int> arr(n),arr1(n);
        for (int i = 0; i < n; ++i) {
            arr[i] = rnd.wnext(min_value, max_value, value_bias);
            arr1[i] = rnd.wnext(min_value, max_value, value_bias);
        }
        cout << n << " " << k << " " << y << "\n";
        println(arr);
        println(arr1);
